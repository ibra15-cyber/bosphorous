...
function ProductScreen() {
  ...
  const { slug } = params; //taking slug out of useParams or making it a variable in params
  //short hand
  //   const slug = useParams.slug //getting the slug from the router
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const { data } = await axios.get(`/api/products/slug/${slug}`); //get data from this link as we await
  ...
  const addToCartHandler = async () => {
    const existItem = cart.cartItems.find((x) => x._id === product._id); //find product using its id
    const quantity = existItem ? existItem.quantity + 1 : 1; //first time item is one, if it exist add+1
    const { data } = await axios.get(`/api/products/${product._id}`); //request a product by it's id
  ...
  return loading ? (
    ...
    <div>
      <Row>
        ...
        <Col md={3}>
          {/**a col 12/3 = 4 looking for 4 items, hence we gave it 4 list items */}
